...
bool WriteFunc(ccmap status)
{
    ...
    std::string xml_out_doc= xml_CMarkup.GetDoc();//将xml转换为字符串
...
bool ReadFunction(ccmap &status)
{
    ...
    status.insert(std::pair<std::string,std::string>("v_head_x0",strSN1));
    xml_CMarkup.FindElem("x1");
    MCD_STR strSN2 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_head_x1",strSN2));
    xml_CMarkup.OutOfElem();
    ...
    status.insert(std::pair<std::string,std::string>("a_body_zhunn",strSN3));
    xml_CMarkup.IntoElem();
    xml_CMarkup.FindElem("st");
    MCD_STR strSN4 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_body_st",strSN4));
    xml_CMarkup.FindElem("mi");
    MCD_STR strSN5=xml_CMarkup.GetAttrib("style");//获取属性值
    status.insert(std::pair<std::string,std::string>("a_body_mi",strSN5));
    xml_CMarkup.IntoElem();
    xml_CMarkup.FindElem("qs");
    MCD_STR strSN6 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_mi_qs",strSN6));
    ...
    status.insert(std::pair<std::string,std::string>("v_mi_cs",strSN7));
    xml_CMarkup.ResetMainPos();
    xml_CMarkup.FindElem("rs");
    MCD_STR strSN8 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_mi_rs",strSN8));
    xml_CMarkup.ResetMainPos();
    xml_CMarkup.FindElem("dk");
    MCD_STR strSN9 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_mi_dk",strSN9));
    xml_CMarkup.ResetMainPos();
    xml_CMarkup.FindElem("ls");
    MCD_STR strSN10 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_mi_ls",strSN10));
    xml_CMarkup.ResetMainPos();
    xml_CMarkup.FindElem("pt");
    MCD_STR strSN11 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_mi_pt",strSN11));
    xml_CMarkup.ResetMainPos();
    xml_CMarkup.FindElem("fj");
    MCD_STR strSN12 = xml_CMarkup.GetData();
    status.insert(std::pair<std::string,std::string>("v_mi_fj",strSN12));
    status.insert(std::pair<std::string,std::string>("test","test"));
    return true;
}
...
int main(int argc, char *argv[])
{
    ...
    writeMap.insert(std::pair<std::string,std::string>("01","0312"));
    writeMap.insert(std::pair<std::string,std::string>("02","9999999"));
    WriteFunc(writeMap);

    ReadFunction(readMap);
    for (iter=readMap.begin();iter!=readMap.end();iter++)
    {
        std::cout << /*setw(10) << */iter->first << "  :   " << iter->second << std::endl;
    }
